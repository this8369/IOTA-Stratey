...
# 1. Revert fetchLogs filter back to just non-members
old_fetch_filter = """            // 1주 전 날짜 계산
...
new_fetch_filter = """            // Filter out non-members ('기타')
...
# 2. Add 1-week filter to filteredLogs
...
new_title = """<h2 className="text-[18px] font-bold text-white tracking-tight translate-y-[2px]">
...
old_button_text = "{logsViewMode === 'summary' ? '전체보기' : '간략히 보기'}"
new_button_text = "{logsViewMode === 'summary' ? '모든 업무 이력 보기' : '최근 1주간 활동 보기'}"
...
with open(path, 'w') as f:
    f.write(content)
print("Filter toggle and text updated.")
