...
export default function Section84({ isActive }) {
    ...
    useEffect(() => {
        ...
        const timers = [
            ...
            setTimeout(() => setStep(2), 680),  // 2026 Box
            setTimeout(() => setStep(3), 1275), // Arrow
            setTimeout(() => setStep(4), 1870), // 2040 Box
            setTimeout(() => setStep(5), 2465)  // Bottom Text
        ];
    ...
    return (
        <section className="section w-full h-full bg-white flex flex-col items-center justify-center relative px-6 md:px-16 overflow-hidden">
            <div className="w-full max-w-[1400px] mx-auto flex flex-col items-center text-center">
                ...
                <h2 className={`text-[32px] md:text-[46px] lg:text-[52px] font-extrabold leading-[calc(1.3em-6px)] text-[#1d1d1f] break-keep tracking-[-0.02em] mb-[54px] transition-all duration-[765ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${step >= 1 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                ...
                <div className="w-full max-w-[1000px] flex flex-col md:flex-row items-center justify-center gap-6 md:gap-12 mb-[54px] relative">
                    ...
                    {/* Current State (2026) */}
                    <div className={`flex flex-col items-center justify-center w-[280px] h-[200px] bg-gray-50 border-2 border-gray-200 rounded-2xl shadow-sm transition-all duration-[600ms] ${step >= 2 ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`}>
                        <div className="text-gray-500 font-bold text-[18px] mb-2">2026 (현재)</div>
                        <div className="text-[#1d1d1f] font-black text-[36px] leading-none mb-1">USD 80<span className="text-[20px] text-gray-500">대</span></div>
                    ...
                    <div className={`hidden md:flex flex-col items-center justify-center transition-all duration-[1020ms] ease-out ${step >= 3 ? 'opacity-100 w-[200px]' : 'opacity-0 w-0'}`}>
                    ...
                    {/* Mobile Arrow */}
                    <div className={`md:hidden flex flex-col items-center justify-center my-4 transition-all duration-[600ms] ${step >= 3 ? 'opacity-100 h-auto' : 'opacity-0 h-0'}`}>
                    ...
                    {/* Future State (2040) */}
                    <div className={`flex flex-col items-center justify-center w-[320px] h-[240px] bg-[#1e3a8a] border-4 border-blue-200 rounded-2xl shadow-2xl relative transition-all duration-[850ms] ${step >= 4 ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`}>
