...
for sheet_name, sheet_df in df.items():
    ...
    for c in existing.data:
        # Delete the ones we inserted earlier, plus any old AUM
        if 'AUM' in c['name'] or '펀드' in c['name'] or '조원' in c['name'] or '부동산' in c['name'] or len(c['name']) > 15 or '[' in c['name']:
    ...
    # 1. AUM data
    # We will combine cells in the row if they start with keywords
    for i in range(min(10, len(sheet_df))):
        ...
        if is_aum and row_vals:
            # Combine them e.g. "프로젝트펀드(모펀드기준): 19,459.5억"
    ...
    for i in range(len(sheet_df)):
        ...
        if is_date:
            ...
            if topic or contents:
                new_records.append({
                    "counterparty_id": cp_id,
                    "name": topic[:255] if topic else "미팅 노트",
                    "title": attendees[:255],
                    "department": igis_attendees[:255],
                    "email": contents[:255], # Put contents in email for old UI compatibility, or metadata
                    "mobile": "",
