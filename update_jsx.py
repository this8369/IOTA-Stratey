...
# Find the tbody and replace conditionally
# It's better to keep the old one and render conditionally:
# {phase421 === 'new' ? ( new_tbody ) : ( old_tbody )}

old_tbody_pattern = re.compile(r'(<tbody className="text-\[13px\] text-\[#E5E5E5\]">.*?</tbody\s*>)', re.DOTALL)
match = old_tbody_pattern.search(content)
