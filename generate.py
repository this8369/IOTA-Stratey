...
def generate_slide_code(name, theme, title, box_title, bullets):
    bullets_html = "\n                        ".join([f'<li className="flex items-start"><span className="mr-3 text-[#1e3a8a] mt-1">▪</span><span>{b}</span></li>' for b in bullets])
    ...
    return (
        <section className="section w-full h-full bg-[#fdfdfd] flex flex-col items-center justify-center relative px-6 md:px-16 overflow-hidden">
            <div className="w-full max-w-[1400px] mx-auto flex flex-col items-center text-center">
                ...
                {{/* Main Title */}}
                <h2 className={{`text-[36px] md:text-[50px] lg:text-[56px] font-extrabold leading-[calc(1.3em-4px)] text-[#1d1d1f] break-keep tracking-[-0.02em] transition-all duration-[900ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${{step >= 2 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}}`}} dangerouslySetInnerHTML={{{{ __html: '{title}' }}}}>
                </h2>
...
for data in slide_data:
    code = generate_slide_code(data["name"], data["theme"], data["title"], data["boxTitle"], data["bullets"])
    with open(f'src/components/{data["name"]}.jsx', 'w', encoding='utf-8') as f:
        f.write(code)

print("Successfully generated Section48 through Section56")
