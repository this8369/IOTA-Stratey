...
for sheet_name, sheet_df in df.items():
    ...
    for c in existing.data:
        # Delete old AUM and Meeting notes
        # Meetings have long names. AUM has specific keywords.
        if 'AUM' in c['name'] or '펀드' in c['name'] or '조원' in c['name'] or '부동산' in c['name'] or len(c['name']) > 15:
    ...
    for i in range(len(sheet_df)):
        ...
        if isinstance(date_val, datetime) or isinstance(date_val, pd.Timestamp):
            ...
            # Extract first match of YYYY-MM-DD
