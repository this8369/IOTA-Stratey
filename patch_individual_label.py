...
content = content.replace("    const displayLabel = workspaceLabel ? workspaceLabel.split('-')[0].trim() : '';", get_cell_name_func)

original_span = """<span className="text-white">{name} {stakeholder?.role_category ? `(${stakeholder.role_category})` : ''}</span>"""
new_span = """<span className="text-white">{name} {getCellName(name) ? `(${getCellName(name)})` : (stakeholder?.role_category ? `(${stakeholder.role_category})` : '')}</span>"""
