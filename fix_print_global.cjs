...
// 1. Rename '공간솔루션/디지털' to '상품·디지털'
content = content.replace(
    "{ id: 'digital', name: '공간솔루션/디지털' }",
    "{ id: 'digital', name: '상품·디지털' }"
);

// 2. Fix print CSS with :global()
...
// Just in case styled-jsx isn't fully honoring :global for pseudo-classes, we can also inject a raw style tag for safety.
// I'll replace the `<style jsx>` with a raw `<style>` tag because the whole block can just be global.
// Wait, if I replace `<style jsx>` with `<style dangerouslySetInnerHTML={{__html: \` ... \`}}>`, it's bulletproof.
const styleJSXStart = `<style jsx>{\``;
const styleJSXEnd = `\`}</style>`;

// Actually, Vite doesn't natively support styled-jsx unless configured. They might be using styled-components or standard CSS?
// The project has `<style jsx>{\` ... \`}</style>`. This is valid in Next.js but in Vite it requires the babel plugin. Since it compiled, it means it works.

fs.writeFileSync(filePath, content, 'utf8');
console.log('Archive print global CSS patched');
