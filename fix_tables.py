...
def fix_file(filepath):
    ...
    # 1. Insert renderDynamicTableBody just above 'const topLps = getTopLps();'
    # Actually, we can insert it right before 'return ('
...
"""
    ...
    # 2. Find the exact block we want to replace
    # We look for:
    # </thead>
    # {phase421 === 'new' ? (
    # ...
    ...
    if start_idx != -1:
        # The block ends exactly at the `)}` before `</table>`
        ...
        if ternary_end_idx != -1:
            ...
            # we need to be careful with the trailing spaces/newlines
            # content[ternary_end_idx:table_close_idx + 8] is ')}\n                        </table>'
            
            content = content[:start_idx] + replacement + content[table_close_idx + 8:]
...
fix_file('src/components/system/SystemFund421.jsx')
fix_file('src/components/system/workspace/WorkspaceFund.jsx')
