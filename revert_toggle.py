...
perfect_toggle = """    const toggle421 = (
        <div className="bg-[#1C1C1E] p-1 rounded-[12px] flex items-center border border-[#3c3c3c]">
            ...
            <button 
                ...
                className={`px-4 py-1.5 rounded-[10px] text-[13px] font-bold transition-all duration-300 cursor-pointer ${phase421 === 'new' ? 'bg-[#2C2C2E] text-white shadow-sm' : 'text-[#86868B] hover:text-white'}`}
            >
                신규 업데이트
            </button>
        </div>
...
for file_path in files:
    ...
    # Finding the end of the toggle block:
    # After toggle421, the next variable is usually activePhase421 or data421 or similar.
    # We can search for the first empty line or specific next line.
    
    # We know the toggle ends with "    );\n"
    # Find the first "    );\n" after start_idx
